...
/**
 * Job: model_10 Purpose: <br>
 * Description:  <br>
 * @author test@talend.com
 * @version 5.5.1.r118616
 * @status 
 */
public class model_10 implements TalendJob {
	...
	private final String jobName = "model_10";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(model_10.this, new Object[] { e,
	...
	public static class out1Struct implements
		...
		final static byte[] commonByteArrayLock_DEMO_FAY_model_10 = new byte[0];
		static byte[] commonByteArray_DEMO_FAY_model_10 = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DEMO_FAY_model_10.length) {
					if (length < 1024
							&& commonByteArray_DEMO_FAY_model_10.length == 0) {
						commonByteArray_DEMO_FAY_model_10 = new byte[1024];
					} else {
						commonByteArray_DEMO_FAY_model_10 = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DEMO_FAY_model_10, 0, length);
				strReturn = new String(commonByteArray_DEMO_FAY_model_10, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DEMO_FAY_model_10) {
	...
	public void tMysqlInput_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				String dbquery_tMysqlInput_1 = "SELECT \r\n  `items_temp`.`id`, \r\n  `items_temp`.`year`, \r\n  `items_temp`.`month`, \r\n  `items_temp`.`city`, \r\n  `items_temp`.`group`, \r\n  `items_temp`.`make`, \r\n  `items_temp`.`premium_segment`, \r\n  `items_temp`.`model_gnr`, \r\n  `items_temp`.`model`, \r\n  `items_temp`.`segment`, \r\n  `items_temp`.`ckd_cbu`, \r\n  `items_temp`.`pc_cv`, \r\n  `items_temp`.`engine_type`, \r\n  `items_temp`.`type`, \r\n  `items_temp`.`displacement`, \r\n  `items_temp`.`sales`, \r\n  `items_temp`.`origine`, \r\n  `items_temp`.`body_type`, \r\n  `items_temp`.`rsp`, \r\n  `items_temp`.`suv_type`, \r\n  `items_temp`.`price_class`, \r\n  `items_temp`.`log_id`, \r\n  `items_temp`.`type_vh`, \r\n  `items_temp`.`status`, \r\n  `items_temp`.`created_at`, \r\n  `items_temp`.`updated_at`,\r\n  `model_pocess9`.`marque`,\r\n  `model_pocess9`.`model`,\r\n  `model_pocess9`.`segument`,\r\n  `model_pocess9`.`body_type`\r\nFROM `items_temp`\r\nINNER JOIN `model_pocess9` ON UPPER(`model_pocess9`.`model`) = UPPER(`items_temp`.`model`) \r\nWHERE `model_pocess9`.`marque` IS NULL OR `model_pocess9`.`marque` = `items_temp`.`make`";
	...
	public static void main(String[] args) {
		final model_10 model_10Class = new model_10();

		int exitCode = model_10Class.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = model_10.class.getClassLoader()
					.getResourceAsStream(
							"demo_fay/model_10_0_1/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : model_10");
		}
...
/************************************************************************************************
 * 133617 characters generated by Talend Open Studio for Data Integration on the
 * November 28, 2015 11:41:07 PM GMT
 ************************************************************************************************/
