...
package aivam.premium_sport_0_1;
...
/**
 * Job: premium_sport Purpose: <br>
 * Description:  <br>
 * @author test@talend.com
 * @version 5.5.1.r118616
 * @status 
 */
public class premium_sport implements TalendJob {
	...
	private final String jobName = "premium_sport";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					premium_sport.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(premium_sport.this, new Object[] { e,
	...
	public static class out1Struct implements
		...
		final static byte[] commonByteArrayLock_AIVAM_premium_sport = new byte[0];
		static byte[] commonByteArray_AIVAM_premium_sport = new byte[0];
		...
		public void copyDataTo(out1Struct other) {
			...
			other.dynamique = this.dynamique;
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_AIVAM_premium_sport.length) {
					if (length < 1024
							&& commonByteArray_AIVAM_premium_sport.length == 0) {
						commonByteArray_AIVAM_premium_sport = new byte[1024];
					} else {
						commonByteArray_AIVAM_premium_sport = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_AIVAM_premium_sport, 0, length);
				strReturn = new String(commonByteArray_AIVAM_premium_sport, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_AIVAM_premium_sport) {
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.dynamique, dos);
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				String update_tMysqlOutput_1 = "UPDATE `"
						+ "items"
						+ "` SET `year` = ?,`month` = ?,`city` = ?,`group` = ?,`make` = ?,`premium_segment` = ?,`model_gnr` = ?,`model` = ?,`segment` = ?,`ckd_cbu` = ?,`pc_cv` = ?,`engine_type` = ?,`type` = ?,`displacement` = ?,`sales` = ?,`origine` = ?,`body_type` = ?,`rsp` = ?,`suv_type` = ?,`price_class` = ?,`log_id` = ?,`type_vh` = ?,`status` = ?,`created_at` = ?,`updated_at` = ?,`dynamique` = ? WHERE `id` = ?";
					...
					while (rs_tMysqlInput_1.next()) {
						...
						if (colQtyInRs_tMysqlInput_1 < 27) {
							...
							tmpContent_tMysqlInput_1 = rs_tMysqlInput_1
									.getString(27);
							if (tmpContent_tMysqlInput_1 != null) {
								row1.dynamique = tmpContent_tMysqlInput_1;
							} else {
								row1.dynamique = null;
							}
						}
						...
						{ // start of Var scope
							...
							out1_tmp.dynamique = row1.dynamique;
							out1 = out1_tmp;
							// ###############################
						...
						if (out1 != null) {
							...
							pstmt_tMysqlOutput_1.setLong(
									27 + count_tMysqlOutput_1, out1.id);
	...
	public void tMysqlInput_2Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				String dbquery_tMysqlInput_2 = "SELECT \r\n  `items`.`id`, \r\n  `items`.`year`, \r\n  `items`.`month`, \r\n  `items`.`city`, \r\n  `items`.`group`, \r\n  `items`.`make`, \r\n  `items`.`premium_segment`, \r\n  `items`.`model_gnr`, \r\n  `items`.`model`, \r\n  `items`.`segment`, \r\n  `items`.`ckd_cbu`, \r\n  `items`.`pc_cv`, \r\n  `items`.`engine_type`, \r\n  `items`.`type`, \r\n  `items`.`displacement`, \r\n  `items`.`sales`, \r\n  `items`.`origine`, \r\n  `items`.`body_type`, \r\n  `items`.`rsp`, \r\n  `items`.`suv_type`, \r\n  `items`.`price_class`, \r\n  `items`.`log_id`, \r\n  `items`.`type_vh`, \r\n  `items`.`status`, \r\n  `items`.`created_at`, \r\n  `items`.`updated_at`,\r\n  `model_mi_sport`.dynamique as 'dynamique'\r\nFROM `items`\r\nINNER JOIN `model_mi_sport` ON  `model_mi_sport`.model = `items`.model AND `model_mi_sport`.`type` \n = `items`.`type`\r\nWHERE  `items`.`make` IN ('AUDI', 'BMW', 'MERCEDES', 'JAGUAR', 'LEXUS', 'VOLVO', 'PORSCHE', 'MINI',\n \r\n  'LAND ROVER','HUMMER')";
					...
					while (rs_tMysqlInput_2.next()) {
						...
						if (colQtyInRs_tMysqlInput_2 < 27) {
							...
							tmpContent_tMysqlInput_2 = rs_tMysqlInput_2
									.getString(27);
							if (tmpContent_tMysqlInput_2 != null) {
								row2.dynamique = tmpContent_tMysqlInput_2;
							} else {
								row2.dynamique = null;
							}
						}
						...
						{ // start of Var scope
							...
							copyOfout1_tmp.dynamique = row2.dynamique;
	...
	public static void main(String[] args) {
		final premium_sport premium_sportClass = new premium_sport();

		int exitCode = premium_sportClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = premium_sport.class
					.getClassLoader().getResourceAsStream(
							"aivam/premium_sport_0_1/contexts/" + contextStr
									+ ".properties");
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : premium_sport");
		}
...
/************************************************************************************************
 * 132725 characters generated by Talend Open Studio for Data Integration on the
